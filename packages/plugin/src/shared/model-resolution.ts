export type ModelHarness = "opencode" | "pi" | "omp" | "dsh";
...
/**
 * OpenCode spells a model qualifier "variant"; Pi and OMP spell it
 * "thinking_level". DSH is an OpenCode-shaped port, so it uses "variant".
 */
function prefersVariantSpelling(harness: ModelHarness): boolean {
...
function resolveHarnessBlock(
    ...
    // "omp" borrows Pi's block; "dsh" is an OpenCode-shaped port, so it
    // inherits the OpenCode block when it has no block of its own.
    const selected =
        harness === "omp"
            ? (container?.omp ?? container?.pi)
            : harness === "dsh"
              ? (container?.dsh ?? container?.opencode)
              : container?.[harness];
...
export function normalizeModelEntry(
    ...
    const qualifier = readString(
        objectEntry[prefersVariantSpelling(harness) ? "variant" : "thinking_level"],
    );
...
export function resolveDreamerTaskModel(args: {
    ...
    const taskQualifier =
        taskBlock?.[prefersVariantSpelling(args.harness) ? "variant" : "thinking_level"];
    ...
    const harnessQualifier =
        harnessBlock[prefersVariantSpelling(args.harness) ? "variant" : "thinking_level"] ??
